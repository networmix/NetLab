...
`netlab metrics` and produces a multi-panel figure comparing median per-iteration
...
def plot_cross_seed_iterops(
    ...
    for ax, col in zip(axes, metrics, strict=False):
        if col not in data.columns:
            ax.axis("off")
            continue
        # Build per-seed scatter from original per-scenario CSVs
        # Fallback to scenario medians if per-seed CSV is missing
        ax.set_title(col)
        ...
        xs = []
        ys = []
        for scen in data.index:
            scen_dir = analysis_root / scen
            p = scen_dir / "iterops_summary.csv"
            if not p.exists():
                continue
            df_s = pd.read_csv(p)
            col_map = {
                "SPF/iter": "spf_calls_total_per_iter",
                "Flows created/iter": "flows_created_total_per_iter",
                "Reopt/iter": "reopt_calls_total_per_iter",
            }
            raw_col = col_map[col]
            if raw_col not in df_s.columns:
                continue
            vals = pd.to_numeric(df_s[raw_col], errors="coerce").values
            for v in vals:
                ...
                ys.append(float(v))
        if xs:
            sns.stripplot(x=xs, y=ys, ax=ax, jitter=0.2, alpha=0.4, color="gray")
        # Scenario median as dot with IQR whiskers
    ...
    fig.suptitle(
        "Iteration operation intensity (distribution across seeds with medians & IQR)"
    )
    fig.tight_layout()
